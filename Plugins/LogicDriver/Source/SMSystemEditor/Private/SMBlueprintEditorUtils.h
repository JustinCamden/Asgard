...
public:
	...
	static TSubclassOf<class UObject> GetNodeTemplateClass(const UEdGraph* ForGraph, bool bReturnDefaultIfNone = false);
	...
	/** Finds the runtime node assicated with a graph and updates it. Looks for any containers references and updates their guid references. */
	...
	/** Searchs for runtime graph nodes with duplicate guids in a blueprint and its parent classes. */
	...
	static UK2Node_CallParentFunction* CreateParentFunctionCall(UEdGraph* Graph, UFunction* ParentFunction, UEdGraphNode* ChildNode, bool bIsDefault = true);
