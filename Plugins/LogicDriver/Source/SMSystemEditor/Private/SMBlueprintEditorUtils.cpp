...
TSubclassOf<UObject> FSMBlueprintEditorUtils::GetNodeTemplateClass(const UEdGraph* ForGraph, bool bReturnDefaultIfNone)
{
	if (USMGraphNode_Base* NodeOwner = FindTopLevelOwningNode(ForGraph))
	{
...
void FSMBlueprintEditorUtils::CleanUpIsolatedTransitions(UEdGraph * Graph)
{
	...
	for (UEdGraphNode* Node : Graph->Nodes)
	{
		if (USMGraphNode_TransitionEdge* Transition = Cast<USMGraphNode_TransitionEdge>(Node))
		{
			...
			if (Transition->GetStartNode() == nullptr || Transition->GetEndNode() == nullptr)
			{
			...
			if (Transition->GetStartNode()->GetGraph() != Graph || Transition->GetEndNode()->GetGraph() != Graph)
			{
				NodesToRemove.Add(Node);
			}
		}
...
UK2Node_CallParentFunction* FSMBlueprintEditorUtils::CreateParentFunctionCall(UEdGraph* Graph, UFunction* ParentFunction, UEdGraphNode* ChildNode, bool bIsDefault)
{
	...
	ParentFunctionNode->NodePosX = ChildNode->NodePosX + ChildNode->NodeWidth + 200;
...
void FSMBlueprintEditorUtils::CollapseNodesAndCreateStateMachine(const TSet<UObject*>& Nodes)
{
			...
			for (UEdGraphPin* Pin : StateNode->Pins)
			{
				...
				for (UEdGraphPin* LinkedPin : Pin->LinkedTo)
				{
					...
					if (USMGraphNode_TransitionEdge* Transition = Cast<USMGraphNode_TransitionEdge>(LinkedPin->GetOwningNode()))
					{
						...
						{
							USMGraphNode_StateNodeBase* OuterNode = Transition->GetStartNode();
						...
						{
							USMGraphNode_StateNodeBase* OuterNode = Transition->GetEndNode();
							if (!Nodes.Contains(OuterNode))
							{
	...
	{
		if (!NewStateMachine->HasTransitionFromNode(Transition->GetStartNode()))
		{
	...
	{
		if (!NewStateMachine->HasTransitionToNode(Transition->GetEndNode()))
		{
	...
	for (UObject* Node : Nodes)
	{
		...
		if (USMGraphNode_TransitionEdge* Transition = Cast<USMGraphNode_TransitionEdge>(Node))
		{
			...
			if (!InnerStates.Contains(Transition->GetStartNode()) || !InnerStates.Contains(Transition->GetEndNode()))
			{
				continue;
			}
		}
